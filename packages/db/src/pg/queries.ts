...
export async function getNetworkGraph(userId: string) {
  ...
  const me = await db.execute(sql`SELECT id, display_name AS "displayName", (avatar IS NOT NULL) AS "hasAvatar" FROM users WHERE id = ${userId}`);
  ...
  type GNode = { id: string; label: string; ring: number; type: "user" | "circle"; hasAvatar?: boolean };
  const nodes: GNode[] = [];
  const links: Array<{ source: string; target: string }> = [];
  const seen = new Set<string>();

  nodes.push({ id: me[0].id as string, label: me[0].displayName as string, ring: 0, type: "user", hasAvatar: !!(me[0].hasAvatar) });
  ...
  for (const c of circlesRows) {
    ...
    const members = await db.execute(sql`
      SELECT u.id, u.display_name AS "displayName", (u.avatar IS NOT NULL) AS "hasAvatar"
    ...
    for (const m of members) {
      ...
      if (!seen.has(mid)) {
        nodes.push({ id: mid, label: m.displayName as string, ring: 2, type: "user", hasAvatar: !!(m.hasAvatar) });
        seen.add(mid);
      }
